...
- **Clarification items with \`answer_status: "not_available_yet"\`** (and the accompanying \`target_master_fields\`, \`claim_limits_es\`, \`confidence_level\`): the user explicitly chose **not** to supply that information yet. You **must** merge this honestly into **input_quality_assessment** (weakness / missing_information / section_scores as appropriate), **evidence_base** (honest gaps, no invented proof), **production_rules** and **guardrails** (cautious claims, no strong promises where evidence is missing), and any **semantic_base** / **voice_base** nuance needed so future content stays credible. **Do not** block synthesis or leave contradictory “resolved” language for those topics.
- If \`post_questionnaire_strategic_refinements.alcohol_responsible_communication_note\` is present, you **must** embed equivalent **responsible-communication** rules in **production_rules** (Spanish strings): no targeting minors; no implying alcohol improves health, status, performance or emotional wellbeing; no encouraging excessive consumption; keep claims on flavor, craft, occasion and responsible enjoyment.
