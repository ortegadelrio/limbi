...
export function buildBrandBaseConsolidationSystemInstructions(): string {
  return [
    ...
    "    – `commercial_use_guidance`: instrucciones claras para equipos y para futuras etapas de producto sobre cómo usar este catálogo al listar servicios en piezas comerciales (sin prometer features). Si `credibility_architecture` tiene contenido sustancial, agregá **una frase** en este campo que conecte el catálogo comercial con la prueba reputacional (sin confundir líneas de servicio con credenciales).",
    ...
    "  • `credibility_architecture` (OBLIGATORIO en v1.2): arquitectura de **credibilidad, autoridad y respaldo reputacional** separada del catálogo comercial.",
    "    – **Fuentes típicas:** `brand_responses` con `question_key` current_evidence y/o `section_key` evidence; a veces approved_source_facts; **no** uses `structured_offer_items` como sustituto de credenciales salvo que el texto explícitamente describa empresas fundadas como **prueba** (no como servicio a vender en catálogo).",
    "    – **Clasificá** en los arrays según el texto: `authority_signals` (señales de autoridad reconocibles en lo dicho), `institutional_roles` (juntas, cargos gremiales, asociaciones), `industry_leadership_assets` (redes sectoriales creadas, eventos propios del sector), `founder_credentials` (fundador/a, organizador/a cuando conste), `business_ecosystem` (empresas o iniciativas fundadas o co-fundadas citadas), `reputation_proof_points` (hechos concretos de trayectoria o respaldo que el usuario aportó).",
    "    – **Preservá nombres propios** tal como aparecen en el contexto (p. ej. UNEMEC, COMARKA, Perrenque Creativo, nombres de agencias o marcas). No inventes entidades, años, premios ni cargos que no estén en las fuentes.",
    "    – **No exageres:** interpretá valor estratégico con sobriedad; no conviertas biografía en autopromoción inflada. Si aparece «líder de industria» u otro claim fuerte solo como autodescripción, reflejalo con cuidado y usá `cautions` para guiar tono (evitar grandilocuencia, sugerir corroboración en piezas públicas cuando corresponda).",
    "    – **`communication_use_guidance`:** explicá cómo usar estos activos en piezas futuras (perfil corporativo, brochure, presentación comercial, bio de conferencista, página «Nosotros», pitch institucional, contenidos de posicionamiento, textos de autoridad sectorial). Debe quedar claro que son **prueba reputacional y respaldo**, no reemplazo del catálogo de servicios.",
    "    – Si no hay señales en el contexto, devolvé arrays vacíos y un `communication_use_guidance` breve y honesto (p. ej. que no hay trayectoria o roles declarados en las fuentes actuales y conviene completar evidencia cuando aplique).",
    "    – `evidence_narrative` y la `section_interpretations` con `section_key` evidence deben **elevar** trayectoria y prueba reputacional cuando existan; no las relegues a párrafo genérico de soporte.",
    ...
    "  • `section_interpretations`: **mínimo 8** objetos, **máximo 14**. Cada uno con **dos capas obligatorias**:",
    "    – `brand_information`: resumen fiel, ordenado y bien redactado de lo que el usuario diligenció en el cuestionario (y mejoras aprobadas de esa sección). Prosa clara, sin jerga de diagnóstico, sin inventar datos. No es interpretación estratégica.",
    "    – `interpretation`: lectura estratégica breve de Limbi sobre esa misma sección (implicaciones, coherencia, foco). No repitas literalmente `brand_information`.",
    "    Además: `section_key` (snake_case del cuestionario), `headline` (titular humano). Debe existir al menos una entrada por: identity, offer, audiences, value_proposition, differentiators, evidence, voice_tone, restrictions.",
    "    Incluí `audiences` fusionando audiencias y territorios cuando el contexto lo permita.",
    "    En `offer`, `brand_information` debe reflejar el catálogo real (`structured_offer_items` / respuestas) y `interpretation` la lectura de cómo se presenta la oferta.",
    "    En `evidence`, `brand_information` lista credenciales, roles y hechos declarados; `interpretation` explica cómo usarlos estratégicamente.",
