...
**Última revisión:** 2026-05-23 — **Ticket B (rediseño Journey de Marca):** migración `20260523120000_ticket_b_brand_journey_question_definitions.sql` desactiva (`is_active = false`) todas las filas previas de `question_definitions` con `journey_type = 'brand'` sin borrarlas, e inserta el catálogo nuevo con `is_active = true`. No hay migración semántica de `brand_responses` viejas. El catálogo **no** incluye `brand_type` (la naturaleza de marca vive en `brand_offer_profiles.offer_nature`), **no** incluye preguntas de oferta ni de territorios (oferta → `brand_offer_items`; territorios → `brand_audience_territories`), **no** incluye `context_sources_note` (material y fuentes siguen siendo paso operativo). Base Límbica: 5 señales; `limbic_emotional_temperature` es `single_choice`. Opciones con `exclusive: true` donde aplica. **Ticket C (pendiente):** opciones “Otro” deben resolverse en UI con campo de especificación o lógica explícita (no bloquea B). **Ticket F (pendiente):** el score del diagnóstico debe ponderar sobre todo obligatorias + datos estructurados esenciales; lo opcional vacío no debe castigar el porcentaje. **Ticket A** sigue vigente: `brand_offer_items`, `brand_audience_territories`, script de reset. **Aún no** en este ticket: UI del cuestionario, APIs de guardado nuevas, análisis documental, diagnóstico, mejora por sección, ejecución del reset, website crawling, `brand_knowledge_bases`, `brand_limbic_bases`, proyectos, generación, contenidos, consolidación final.
...
- **Journey de Marca completo:** parcial. Tickets 1–5 cubren marca, catálogo, `brand_responses`, documentos + `brand_source_facts`, **`brand_evaluations` (diagnóstico)** y **mejora por sección** (`brand_improvement_sessions` / `brand_section_improvements`). Ticket A agrega estructura para oferta y territorios; **Ticket B** aplica el nuevo catálogo activo de `question_definitions`. Aún faltan UI nueva alineada al rediseño, APIs específicas de guardado, actualización de IA, `brand_knowledge_bases`, `brand_limbic_bases` y **consolidación** en bases activas.
