...
**Última revisión:** 2026-05-12 — **Ticket G:** mejora por sección alineada al Journey v2 (`brand-section-improvement-v2.0`, `brand-section-improve-context-v2.0`). Contexto con `brand_offer_profile`, `structured_offer_items`, `structured_audience_territories`, `scoring_policy`, `structured_context_note`, respuestas filtradas al catálogo activo, coherencia sin huérfanas, diagnóstico de sección con `depth_opportunities`; prompt distingue esencial vs opcional, negativos como restricciones y Base Límbica simbólica. Aprobación enriquece `brand_section_improvements.payload` (resumen, trazas, `should_mark_as_ready_for_consolidation`, etc.) sin tocar `brand_responses` ni tablas estructuradas; `brands.updated_at` se actualiza al aprobar para staleness. UI: etiqueta humana, diagnóstico por sección con bloques esencial/opcional/riesgos, panel inventario/territorios. **Ticket F:** diagnóstico de marca alineado al nuevo Journey (`brand-diagnosis-v2.0`, `brand-diagnosis-context-v2.0`). El contexto incluye `structured_offer_items`, `structured_audience_territories` y `scoring_policy`; `brand_responses` se filtra al catálogo activo; solo `brand_source_facts` aprobados entran al JSON; mejoras aprobadas activas siguen con prioridad contextual. Scoring: obligatorias + datos estructurados esenciales; suelo servidor cuando todas las obligatorias están respondidas pero quedan opcionales vacías (`applyOptionalEmptinessSectionScoreFloors`); opcionales no castigan duro. UI distingue vacíos esenciales vs profundización opcional; riesgos enmarcados como restricciones; sin `section_key` visibles al usuario (solo etiquetas). Staleness: cualquier `updated_at` en `brand_source_facts` tras el diagnóstico marca desactualizado (aprobación/rechazo/edición). **Ticket E:** análisis documental alineado al Journey de Marca actual (`brand-document-analysis-v2.0` en `lib/schemas/brand-document-analysis.ts` y `lib/prompts/brand-document-analysis.ts`). Los hallazgos siguen siendo solo `brand_source_facts` pendientes de revisión; no se escribe en `brand_responses`, `brand_offer_items` ni `brand_audience_territories` desde el análisis. Dedupe ampliado con inventario/territorios estructurados ya guardados. Negativos/restricciones y Base Límbica simbólica descritos en el prompt. **Ticket B (rediseño Journey de Marca):** migración `20260523120000_ticket_b_brand_journey_question_definitions.sql` desactiva (`is_active = false`) todas las filas previas de `question_definitions` con `journey_type = 'brand'` sin borrarlas, e inserta el catálogo nuevo con `is_active = true`. No hay migración semántica de `brand_responses` viejas. El catálogo **no** incluye `brand_type` (la naturaleza de marca vive en `brand_offer_profiles.offer_nature`), **no** incluye preguntas de oferta ni de territorios (oferta → `brand_offer_items`; territorios → `brand_audience_territories`), **no** incluye `context_sources_note` (material y fuentes siguen siendo paso operativo). Base Límbica: 5 señales; `limbic_emotional_temperature` es `single_choice`. Opciones con `exclusive: true` donde aplica. **Tickets C / C.1** (UI cuestionario nueva, oferta/territorios en tablas, Base Límbica, “Otro” en `answer_value`, pulido UX): hechos en código vigente. **Ticket A** sigue vigente: `brand_offer_items`, `brand_audience_territories`, script de reset. **Aún no:** consolidación en `brand_knowledge_bases` / `brand_limbic_bases`, `force` / reanálisis explícito, cola async de análisis, ejecución del reset, website crawling, proyectos/generación alineados a bases marca.
...
- **Journey de Marca completo:** parcial. Tickets 1–5 cubren marca, catálogo, `brand_responses`, documentos + `brand_source_facts`, **`brand_evaluations` (diagnóstico)** y **mejora por sección** (`brand_improvement_sessions` / `brand_section_improvements`). Ticket A agrega estructura para oferta y territorios; **Ticket B** aplica el nuevo catálogo activo de `question_definitions`; **Ticket E** alinea el análisis documental a esa taxonomía. Aún faltan `brand_knowledge_bases`, `brand_limbic_bases` y **consolidación** en bases activas.
...
- **Ticket F (diagnóstico Journey de Marca v2):** `brand-diagnosis-v2.0` + `brand-diagnosis-context-v2.0`; contexto con `structured_offer_items`, `structured_audience_territories`, filtrado de `brand_responses` al catálogo activo, `scoring_policy`; suelo de score por opcionales vacíos; staleness ampliado a cambios en cualquier `brand_source_facts`; UI diferencia esencial vs profundización opcional.
- **Ticket 4:** `brand_evaluations` + diagnóstico (`build-brand-diagnosis-context`, APIs, UI `/brands/[brandId]/diagnosis`).
- **Ticket G + Ticket 5 (mejora por sección):** tablas `brand_improvement_sessions`, `brand_improvement_messages`, `brand_section_improvements`; ruta `/improve/[sectionKey]`. **Ticket G** añade `brand-section-improvement-v2.0`, `brand-section-improve-context-v2.0`, contexto con oferta/territorios estructurados y diagnóstico v2 por sección, filtrado de respuestas al catálogo activo, payload de aprobación enriquecido y `brands.updated_at` al aprobar; **no** escribe en `brand_responses`, `brand_offer_items` ni `brand_audience_territories`.
